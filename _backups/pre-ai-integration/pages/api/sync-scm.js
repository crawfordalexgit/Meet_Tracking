...
import { fetchScmNumericIds, scmLogin, fetchSwimmerSquadJoinDate } from '../../lib/scm-scraper';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sessionsData.length > 0) {
      const sessionsToUpsert = sessionsData.map(s => {
    ...
    const { data: existingSwimmers } = await supabase
      .from('swimmers')
    ...
    const swimmersToInsert = membersData
      ...
      .map(member => {
        ...
        // Default to existing date or today if new
        let joinDate = existing ? existing.squad_join_date : new Date().toISOString().split('T')[0];
    ...
    if (finalSwimmersToInsert.length > 0) {
      ...
      if (process.env.SCM_WEB_USERNAME && process.env.SCM_WEB_PASSWORD) {
        try {
          ...
          if (swimmersToVerify && swimmersToVerify.length > 0) {
            console.log(`Verifying join dates for ${swimmersToVerify.length} swimmers...`);
