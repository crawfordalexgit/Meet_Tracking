export default function PremiumOrb({ value = 0, label, size = 120, trend, icon, customValue }) {
  const safeValue = Math.min(100, Math.max(0, value));
  const r = size * 0.4;
  const cx = size / 2;
  const cy = size / 2;
  const circ = 2 * Math.PI * r;
  const offset = circ * (1 - (customValue ? (typeof value === 'number' && value > 0 ? 100 : 0) : safeValue) / 100);
  ...
  const theme = getTheme(customValue ? (typeof value === 'number' && value > 0 ? 100 : 20) : safeValue);
