...
export default function SquadsRegistry({ session }) {
  ...
  const fetchData = async () => {
    ...
    try {
      // 1. Fetch data
      const { data: sData } = await supabase.from('squads').select('*').eq('is_squad', true).order('name');
      const { data: swData } = await supabase.from('swimmers').select('*').eq('is_active', true);
      const { data: rData } = await supabase.from('results').select('*, meets(id,name,type)').gte('date', new Date(new Date() - 365 * 86400000).toISOString());
      const { data: aData } = await supabase.from('attendance').select('*').gte('date', new Date(new Date() - 90 * 86400000).toISOString());
      const { data: sessions } = await supabase.from('sessions').select('*');
      const { data: memberships } = await supabase.from('session_memberships').select('*');
      const { data: exemptions } = await supabase.from('exemptions').select('*');
      const { data: rankings } = await supabase.from('rankings').select('*').order('snapshot_date', { ascending: false });
      ...
      const squadsArr = sData || [];
      const swimmersArr = swData || [];
      const resultsArr = rData || [];
      const attendanceArr = aData || [];
      const sessionsArr = sessions || [];
      const membershipsArr = memberships || [];
      const exemptionsArr = exemptions || [];
