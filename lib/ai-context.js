...
import { getRankingReferenceYear, getSeasonAge } from './season';
...
export async function getSwimmerDNA(swimmer, results, attendance, sessions, feedback = [], performance_slope = 0, period = 365) {
  ...
  // Benchmarks come from the rankings scrape, so they are keyed to the year that
  // scrape bucketed age groups by (the current calendar year) and their
  // age_group is the age at 31 December of it — the lookup age must use the same
  // reference, not the target championship season.
  const rankingYear = getRankingReferenceYear();
  const compAge = getSeasonAge(swimmer.year_of_birth, rankingYear);
  const serviceSupabase = getServiceSupabase();
  
  const [benchmarksRes, membershipsRes, exemptionsRes, rankingsRes, pbsRes, squadsRes] = await Promise.all([
    compAge ? serviceSupabase.from('benchmarks').select('*').eq('gender', swimmer.gender).eq('age_group', compAge).eq('year', rankingYear) : { data: [] },
    ...
    serviceSupabase.from('swimmer_pbs').select('*').eq('swimmer_id', swimmer.id),
    serviceSupabase.from('squads').select('*')
  ]);
  ...
  primaryStrokes.forEach(stroke => {
    ['LC', 'SC'].forEach(course => {
      ...
      const twoHundred = sortedResults.find(r => r.event === `200m ${stroke}` && (isLongCourse(r) ? 'LC' : 'SC') === course);
      if (fifty && hundred) {
      ...
      if (hundred && twoHundred) {
        const hundredSec = parseTimeToSeconds(hundred.time);
        const twoHundredSec = parseTimeToSeconds(twoHundred.time);
        ratios[`${stroke}_100_200_${course}`] = (twoHundredSec / (hundredSec * 2)).toFixed(3);
      }
    });
  });

  // Calculate True In-Race Pacing Ratios
  const inRacePacing = [];
  swimmerPBs.forEach(pb => {
    if (pb.splits && typeof pb.splits === 'object') {
      const distances = Object.keys(pb.splits).map(Number).sort((a, b) => a - b);
      if (distances.length >= 2) {
        const maxDist = distances[distances.length - 1];
        const midDist = maxDist / 2;
        const midSplit = distances.find(d => d === midDist) || distances;

        const firstHalfTime = pb.splits[midSplit]?.cumulative;
        const totalTime = pb.splits[maxDist]?.cumulative;

        const firstHalfSec = parseTimeToSeconds(firstHalfTime);
        const totalSec = parseTimeToSeconds(totalTime);
        const secondHalfSec = totalSec - firstHalfSec;

        if (firstHalfSec > 0 && totalSec > firstHalfSec) {
          const ratio = (secondHalfSec / firstHalfSec).toFixed(3);
          inRacePacing.push({
            event: pb.event,
            course: pb.course,
            first_half_seconds: firstHalfSec,
            second_half_seconds: secondHalfSec,
            total_seconds: totalSec,
            ratio: parseFloat(ratio),
            status: ratio > 1.15 ? "HEAVY POSITIVE SPLIT" : (ratio < 0.98 ? "NEGATIVE SPLIT" : "OPTIMAL PACING")
          });
        }
      }
    }
  ...
  return {
    ...
    training: {
      ...
      average_weekly_pool_hours: reliability.totalWeeks > 0 ? Number((reliability.totalTrainingHours / reliability.totalWeeks).toFixed(1)) : 0,
      average_weekly_gala_hours: reliability.totalWeeks > 0 ? Number((reliability.totalGalaHours / reliability.totalWeeks).toFixed(1)) : 0,
      average_weekly_hours: reliability.totalWeeks > 0 ? Number((reliability.totalHours / reliability.totalWeeks).toFixed(1)) : 0,

      recent_workload_last_4_weeks: Object.entries(workload)
        .sort((a, b) => new Date(b[0]) - new Date(a[0])) // Newest first
        .slice(0, 4)
        .map(([date, data]) => ({ date, hours: (data.trainingHours || 0) + (data.galaHours || 0) }))
    },
    ...
    squad_configurations: allSquads.map(s => {
      let normalizedName = s.name;
      const nameUpper = s.name.toUpperCase();
      if (nameUpper.includes('BRONZE')) normalizedName = 'Bronze';
      else if (nameUpper.includes('SILVER')) normalizedName = 'Silver';
      else if (nameUpper.includes('GOLD DEVELOPMENT')) normalizedName = 'Gold Development';
      else if (nameUpper.includes('AGE DEVELOPMENT')) normalizedName = 'Age Development';
      else if (nameUpper === 'NAR') normalizedName = 'NAR (Nationals & Regionals)';
      else if (nameUpper.includes('TECHNICAL DEVELOPMENT')) normalizedName = 'Technical Development (14 and younger)';
      else if (nameUpper.includes('CLUB 2')) normalizedName = 'Club 2 (for 14+)';

      return {
        name: s.name,
        normalized_name: normalizedName,
        target_hours: s.target_hours_per_week || 0,
        target_sessions: s.target_sessions_per_week || 0
      };
    })
  };
}
