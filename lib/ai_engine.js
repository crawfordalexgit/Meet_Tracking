...
export async function analyzeFacet(dna, facet, customPrompt = null, type = 'general', instructions = null) {
  ...
  if (dataDna.staff_context) {
    // Length only, no preview: coaching notes name individual swimmers, and
    // this goes to the platform log.
    console.log(`>>> AI ENGINE: Staff context detected (${dataDna.staff_context.length} chars).`);
...
${pdfSection}`;
  ...
  // Length only: the gala PDF text is full of swimmers' names.
  if (pdfSection) console.log(`>>> AI ENGINE: PDF evidence attached (${pdfSection.length} chars).`);
...
export async function chatWithAssistant(history, clubDNA) {
  ...
  return aiChat({ history, systemPrompt: fullSystemPrompt });
}
