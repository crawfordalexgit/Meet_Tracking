...
import { closureOn, closureCoversVenue, describeClosure } from './venue-closures.js';
 ...
 * bank holiday.
 *
 * A closure can name a venue, in which case it only excuses the sessions in
 * that water. The club lost the town pool for ten days and kept training
 * everywhere else; excusing every session in the club for it would have hidden
 * the ones that really were missed.
 */
export function runDatesFor(dayOfWeek, from, to, closures = [], venue = null) {
  ...
  const shut = (key) => closureOn(key, venue, closures) !== null;
...
export function assessSession(session, marks, options = {}) {
  ...
  const venue = session.location || session.venue || null;
  const runs = runDatesFor(session.day, from, to, closures, venue);

  // Why some nights are not being asked about. A coach reading "8 of 11" needs
  // to see the three that were written off and by whose decision.
  const excusedBy = (closures || []).filter(c =>
    c && c.type === 'exempt' && c.venue && closureCoversVenue(c, venue) &&
    c.start_date <= to && c.end_date >= from);
  const byDate = {};
  (marks || []).forEach(m => {
    if (!m || !m.date) return;
  ...
  return {
    ...
    daysSince,
    flags,
    // Nights written off because the pool was gone, named so a coach can see
    // why the count is lower than the weeks in the window.
    excusedByVenue: excusedBy.map(c => ({
      name: c.name || 'Venue closed',
      venue: c.venue,
      from: c.start_date,
      to: c.end_date,
      detail: describeClosure(c)
    })),
 ...
 *
 * Learn to Swim and land training are not squads, but they are groups with
 * coaches and registers of their own, and filing them under "No squad" hid
 * five sessions behind a heading that reads like a data fault. They are named
 * for what they are.
 */
export function squadNameFor(sessionName, squadNames = []) {
  const n = String(sessionName || '').toLowerCase();
  if (!n) return null;
  if (/\blts\b|learn to swim/.test(n)) return 'Learn to Swim';
  if (/land training/.test(n)) return 'Land training';
