...
/**
 * Resolves a session's weekday. sessions.day_of_week is NULL on every row in the
 * database, so the day is recovered from the session name when the column is empty.
 */
export function extractSessionDay(session) {
  const dw = session?.day_of_week;
  if (dw) {
    const match = DAY_NAMES_FULL.find(d => d.toLowerCase() === dw.toLowerCase());
    if (match) return match;
  }
  if (session?.name) {
    const match = DAY_NAMES_FULL.find(d => session.name.toLowerCase().includes(d.toLowerCase()));
    if (match) return match;
  }
  return dw || 'Unknown';
}

export function getDayOrder(day) {
  const idx = DAY_NAMES_FULL.findIndex(d => d.toLowerCase() === (day || '').toLowerCase());
  return idx === -1 ? 99 : idx;
}
...
export function calculateReliability(swimmer, attendance, sessions, results, periodDays = 365, exemptions = [], sessionMemberships = [], filterOptions = {}) {
  ...
  // Force every entry to type:'exempt' — neutralizes any ghost-credit records at runtime.
  const allExemptions = [...(exemptions || []), ...DEFAULT_EXEMPTIONS].map(ex => ({ ...ex, type: 'exempt' }));
   ...
   for (let i = 0; i < totalWeeksCount; i++) {
     ...
     if (!workloadByWeek[weekKey]) {
      ...
      for (let d = 0; d < 7; d++) {
          ...
          const allExemptions = [...(exemptions || []), ...DEFAULT_EXEMPTIONS];
          
          // CRITICAL FIX 1: Explicitly search for a 'credit' type
          const validCredit = allExemptions.find(ex => 
      ...
      if (calculatedWeekCredits > baseTargetHrs) calculatedWeekCredits = baseTargetHrs;
     ...
     let isExemptWeek = false;
      ...
      for (let dIdx = 0; dIdx < 7; dIdx++) {
        ...
        const shutdown = isShutdownDate(dateStr, allExemptions, swimmer.squad_id);
...
export function computeSquadStats(squad, squadSwimmers, { attendance = [], sessions = [], results = [], memberships = [], exemptions = [], period = 365 } = {}) {
  ...
  const training = Math.round(per.reduce((a, b) => a + b.percentage, 0) / n);
  const volume = Math.round(per.reduce((a, b) => a + b.volumePct, 0) / n);
  const avgVelocity = Math.round(per.reduce((a, b) => a + b.velocity, 0) / n);
  const avgPts = Math.round(per.reduce((a, b) => a + b.peakWA, 0) / n);
