...
export function generateNameAliases(swimmer) {
  ...
  // Calculate a reliable Last Name ("Last, First" or "First Last")
...
export function normalizeEvent(event) {
  ...
  // Remove HY-TEK age group ranges like "14-99", "8-11", "12-13"
  e = e.replace(/\d+[-–]\d+\s*/g, '');

  // Remove "N Year Olds" age group (e.g. "9 Year Olds", "12 Year Olds")
  e = e.replace(/\d{1,2}\s+year\s*olds?\s*/gi, '');
  // Clean up orphaned "Year Olds" left after range strip (e.g. "16-99 Year Olds" → "Year Olds")
  e = e.replace(/year\s*olds?\s*/gi, '');
...
export function getWeekKey(date) {
    let d;
    // CRITICAL FIX: Parse string manually into local time to prevent UTC midnight shifting backwards
...
export function toLocalISO(date) {
    if (!date) return '';
    // CRITICAL FIX: If already a YYYY-MM-DD string, return it immediately to prevent UTC timezone shifts!
...
/**
 * Built-in UK bank holidays and club shutdowns.
 * All entries use type: 'exempt' — weeks are removed from the swimmable denominator
 * rather than awarding a 100% ghost-credit pass.
 */
export const DEFAULT_EXEMPTIONS = [
  // 2025 August Shutdown
  { name: 'August Shutdown', start_date: '2025-08-25', end_date: '2025-08-31', type: 'exempt' },
  // 2026 August Shutdown
  { name: 'August Shutdown', start_date: '2026-08-25', end_date: '2026-08-31', type: 'exempt' },

  // 2025 Christmas Shutdown
  { name: 'Christmas Shutdown', start_date: '2025-12-22', end_date: '2025-12-28', type: 'exempt' },
  // 2026 Christmas Shutdown
  { name: 'Christmas Shutdown', start_date: '2026-12-22', end_date: '2026-12-28', type: 'exempt' },

  // 2025 Bank Holidays
  { name: 'New Year Bank Holiday', start_date: '2025-01-01', end_date: '2025-01-01', type: 'exempt' },
  { name: 'Good Friday',           start_date: '2025-04-18', end_date: '2025-04-18', type: 'exempt' },
  { name: 'Easter Monday',         start_date: '2025-04-21', end_date: '2025-04-21', type: 'exempt' },
  { name: 'Early May Bank Holiday',start_date: '2025-05-05', end_date: '2025-05-05', type: 'exempt' },
  { name: 'Spring Bank Holiday',   start_date: '2025-05-26', end_date: '2025-05-26', type: 'exempt' },
  { name: 'Summer Bank Holiday',   start_date: '2025-08-25', end_date: '2025-08-25', type: 'exempt' },
  { name: 'Christmas Day',         start_date: '2025-12-25', end_date: '2025-12-25', type: 'exempt' },
  { name: 'Boxing Day',            start_date: '2025-12-26', end_date: '2025-12-26', type: 'exempt' },

  // 2026 Bank Holidays
  { name: 'New Year Bank Holiday', start_date: '2026-01-01', end_date: '2026-01-01', type: 'exempt' },
  { name: 'Good Friday',           start_date: '2026-04-03', end_date: '2026-04-03', type: 'exempt' },
  { name: 'Easter Monday',         start_date: '2026-04-06', end_date: '2026-04-06', type: 'exempt' },
  { name: 'Early May Bank Holiday',start_date: '2026-05-04', end_date: '2026-05-04', type: 'exempt' },
  { name: 'Spring Bank Holiday',   start_date: '2026-05-25', end_date: '2026-05-25', type: 'exempt' },
  { name: 'Summer Bank Holiday',   start_date: '2026-08-31', end_date: '2026-08-31', type: 'exempt' },
  { name: 'Christmas Day',         start_date: '2026-12-25', end_date: '2026-12-25', type: 'exempt' },
  { name: 'Boxing Day',            start_date: '2026-12-28', end_date: '2026-12-28', type: 'exempt' },

  // Easter School Holidays
  { name: 'Easter Holidays', start_date: '2025-04-14', end_date: '2025-04-17', type: 'exempt' },
  { name: 'Easter Holidays', start_date: '2026-03-30', end_date: '2026-04-02', type: 'exempt' },
];
...
export function calculateWorkload(attendance, sessions, results, swimmerId, exemptions = [], squadId = null) {
  ...
  attendance.filter(a => a.swimmer_id === swimmerId).forEach(att => {
  ...
  if (results) {
    const attendanceDates = new Set(attendance.filter(a => a.swimmer_id === swimmerId).map(a => a.date));
    const uniqueResultDatesSet = new Set();

    // Credit only actual race dates (r.date), not the full meet date range.
    // A swimmer at a 3-day meet should get 2.0h credit per day they actually raced,
    // not per calendar day the meet ran.
    results.filter(r => r.swimmer_id === swimmerId).forEach(r => {
      if (r.date) uniqueResultDatesSet.add(r.date);
    });
...
export function calculateReliability(swimmer, attendance, sessions, results, periodDays = 365, exemptions = [], sessionMemberships = [], filterOptions = {}) {
  ...
  // Merge caller-supplied exemptions with built-in bank holidays / shutdowns.
  // Two views of the same list:
  //  - allExemptionsTyped keeps the real type, so the 'credit' branch below can
  //    actually fire. Forcing everything to 'exempt' here made that branch
  //    unreachable and silently disabled the Shutdown Credits feature described
  //    in AttendanceLogic.readme.
  //  - allExemptions is the exempt-only view, for the places that only ask
  //    "was this day written off?".
  const allExemptionsTyped = [...(exemptions || []), ...DEFAULT_EXEMPTIONS];
  const allExemptions = allExemptionsTyped.map(ex => ({ ...ex, type: 'exempt' }));
  ...
  // CRITICAL FIX: Strictly trust the database squad_join_date. Do not infer from attendance.
  ...
  const totalWeeksCount = Math.floor(daysInSquad / 7);
   ...
   const workloadByWeek = calculateWorkload(attendance, sessions, results, swimmer.id, allExemptions, swimmer.squad_id);
   ...
   // Safety: only allow holiday week credits for swimmers who have actually trained.
   // Prevents a swimmer with zero attendance getting a free perfect score via holidays alone.
   const hasTrainingHistory = attendance.some(a => a.swimmer_id === swimmer.id && a.status === 'present');
   ...
   for (let i = 0; i < totalWeeksCount; i++) {
     ...
     if (!workloadByWeek[weekKey]) {
      ...
      let calculatedWeekCredits = 0;

      const [yyyy, mm, dd] = weekKey.replace('W-', '').split('-');
      const weekMonday = new Date(parseInt(yyyy), parseInt(mm) - 1, parseInt(dd));
      const dayNamesLower = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
      
      let creditReasons = [];

      for (let d = 0; d < 7; d++) {
          // CRITICAL FIX: Use Native Date math instead of 24h millisecond blocks to avoid Daylight Saving Time shifts
          const dayDate = new Date(weekMonday.getFullYear(), weekMonday.getMonth(), weekMonday.getDate() + d);
          const dateObj = toLocalISO(dayDate);
          // CRITICAL FIX 1: Explicitly search for a 'credit' type
          const validCredit = allExemptionsTyped.find(ex =>
              dateObj >= ex.start_date && 
              dateObj <= ex.end_date && 
              (!ex.squad_id || ex.squad_id === swimmer.squad_id) &&
              ex.type === 'credit'
          );
          
          if (validCredit) {
              const currentDayName = dayNamesLower[dayDate.getDay()];
              let dailyCreditHours = 0;
              
              if (sessionMemberships && sessionMemberships.length > 0) {
                  let mappedDays = [];
                  sessionMemberships.forEach(m => {
                      const sess = sessions.find(s => s.id === m.session_id);
                      if (sess && sess.day_of_week) {
                          const dbDay = sess.day_of_week.toLowerCase().trim();
                          mappedDays.push(dbDay);
                          if (currentDayName === dbDay || currentDayName.includes(dbDay) || dbDay.includes(currentDayName)) {
                              dailyCreditHours += getSessionDuration(sess);
                          }
                      } else if (sess) {
                          mappedDays.push('NoDayDefined');
                      } else {
                          mappedDays.push('MissingSessionID');
                      }
                  });
                  
                  if (dailyCreditHours > 0) {
                      creditReasons.push(`${validCredit.name} (${dailyCreditHours.toFixed(1)}h matched on ${currentDayName})`);
                  } else {
                      // Diagnostic Reason: Show exactly why it failed to grant hours
                      creditReasons.push(`${validCredit.name} (0.0h - Closed on ${currentDayName}. Swimmer sessions: ${mappedDays.join(', ') || 'None'})`);
                  }
              } else {
                  dailyCreditHours = (baseTargetHrs / 7);
                  creditReasons.push(`${validCredit.name} (${dailyCreditHours.toFixed(1)}h - Flat Rate Applied)`);
              }
              
              calculatedWeekCredits += dailyCreditHours;
          }
      }
      
      // Clamp against THIS week's target, not the squad base — an age-based
      // rule can raise or lower the week's hours.
      if (calculatedWeekCredits > targetHrs) calculatedWeekCredits = targetHrs;
      weekData.creditedHours = calculatedWeekCredits;
      weekData.credit = calculatedWeekCredits;
      weekData.creditReasons = creditReasons;
      weekData.totalHours = weekData.trainingHours || 0;
     ...
     let isExemptWeek = false;
      ...
      for (let dIdx = 0; dIdx < 7; dIdx++) {
        ...
        // Typed view: this block branches on 'exempt' vs 'credit'.
        const shutdown = isShutdownDate(dateStr, allExemptionsTyped, swimmer.squad_id);
          ...
          if (isScheduled) {
            if (sessionMemberships && sessionMemberships.length > 0) {
              ...
              const relevantSessions = sessions.filter(s => {
                const sName = s.name?.toUpperCase() || "";
                const sDayField = (s.day_of_week || "").toLowerCase();
                // squadName.includes(squadName) was a tautology, so this matched
                // EVERY session in the club for every swimmer.
      ...
      if (isExemptWeek) {
       ...
       // Credit actual hours earned during exempt weeks (holiday camps, make-up swims, etc.)
       // Exemption removes the week from the denominator — it must NOT erase real work done.
       // Volume accumulators are ungated — includeGalas controls consistency metrics only.
       const wExempt = workloadByWeek[weekKey];
       totalTrainingHours += wExempt.trainingHours || 0;
       totalGalaHours += wExempt.galaHours || 0;
       continue;
     }
     ...
     if (w || creditedSessionsInWeek > 0) {
       ...
       // VOLUME: always accumulate raw hours regardless of includeGalas filter.
       // activeGalaHours (gated) is used only for consistency checks (sessMet/hrsMet) below.
       ...
       const hrsMet = (work.trainingHours + activeGalaHours + creditedHoursInWeek + (work.creditedHours || 0)) >= targetHrs;
       ...
       if (creditedSessionsInWeek > 0) {
          for (let dIdx = 0; dIdx < 7; dIdx++) {
            ...
            if (day === 0 || day === 6) {
               const dateStr = toLocalISO(currentDate);
               const sd = isShutdownDate(dateStr, allExemptions, swimmer.squad_id);
       ...
       if (includeHolidays && hasTrainingHistory && !isMet && (work.trainingSessions + work.galaSessions + creditedSessionsInWeek) === 0 && holidaysUsed < holidayAllowance) {
         isMet = true;
         holidayUsed = true;
         holidaysUsed++;
       }
     ...
     } else if (includeHolidays && hasTrainingHistory && holidaysUsed < holidayAllowance) {
       // NO RECORD AT ALL: Use a holiday (only if swimmer has trained at least once)
       holidaysUsed++;
       weeksMetCount++;
       hoursMetCount++;
       sessionsMetCount++;
  ...
  // Meet Compliance
   // Meet Compliance: Seasonal target (do not scale down by weeks to match settings)
    ...
    const seasonalMeetTarget = squad.target_meets || 5;
    // Prorate target to the analysis period (e.g. 90-day window → ~25% of annual target)
    const targetMeets = Math.round((seasonalMeetTarget * (periodDays / 365)) * 10) / 10;
  ...
  return {
    // No swimmable week means we know nothing about this swimmer, not that they
    // were perfect. Returning 100 rendered every brand-new athlete as fully
    // compliant. null lets callers show '—' and keeps them out of the
    // ">= target" compliance comparisons.
    hasReliabilityData: swimmableWeeks > 0,
    percentage: swimmableWeeks > 0 ? Math.min(100, Math.round((weeksMetCount / swimmableWeeks) * 100)) : null,
    hoursCompliance: swimmableWeeks > 0 ? Math.min(100, Math.round((hoursMetCount / swimmableWeeks) * 100)) : null,
    sessionsCompliance: swimmableWeeks > 0 ? Math.min(100, Math.round((sessionsMetCount / swimmableWeeks) * 100)) : null,
    ...
    // workloadByWeek is keyed by getWeekKey ("W-YYYY-MM-DD"); this used to look
    // up a bare toLocalISO date ("YYYY-MM-DD"), so it never matched and every
    // caller silently fell back to the squad base targets.
    ...(() => {
      const lastWeek = workloadByWeek[getWeekKey(new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000))];
      return {
        targetSess: lastWeek?.requiredSessions ?? baseTargetSess,
        targetHrs: lastWeek?.target ?? baseTargetHrs,
        appliedRule: lastWeek?.appliedRule || null,
      };
    })()
  };
}
...
export function generateSwimmerNarrative(stats, swimmer) {
  ...
  if (stats.totalWeeks > 0) {
      const hourDiff = stats.totalActualHours - stats.annualTargetHours;
      workloadText = `Consistency: You have met the squad criteria in ${stats.weeksMet} out of ${stats.totalWeeks} swimmable weeks (${weeksMetPct}%). Your average volume is ${avgWeeklyHours}h per week. In total, you have completed ${Math.round(stats.totalActualHours)}h out of a target ${Math.round(stats.annualTargetHours)}h.`;
      
      if (hourDiff >= 0) {
          workloadText += ` You have exceeded your seasonal volume target by ${Math.round(hourDiff)}h, successfully banking a massive aerobic base.`;
      } else {
          workloadText += ` This ${Math.round(Math.abs(hourDiff))}h deficit directly impacts your aerobic ceiling.`;
      }
  } else {
  ...
  if (stats.strokeData && Object.keys(stats.strokeData).length > 0) {
    Object.entries(stats.strokeData)
      ...
      .forEach(([stroke, data]) => {
        ...
        if (peak >= rTarget) {
          strokeType = "success";
          strokeStatus = `Your peak performance of ${peak} WA points is exceeding the Regional automatic standard (${rTarget} pts). This marks ${stroke} as an elite championship discipline.`;
        } else if (peak >= cTarget) {
          strokeType = "success";
          const gapToReg = rTarget - peak;
          strokeStatus = `Your peak performance of ${peak} WA points is exceeding the County standard (${cTarget} pts). You are chasing the Regional standard (${rTarget} pts) with a remaining gap of just ${gapToReg} pts.`;
        } else if (peak > cTarget - 30) {
          strokeType = "warning";
          const gapToCounty = cTarget - peak;
          strokeStatus = `Your peak performance of ${peak} WA points is close to the County baseline of ${cTarget} pts. Chasing the standard with a narrow gap of ${gapToCounty} pts.`;
        } else {
          strokeType = "danger";
          const gapToCounty = cTarget - peak;
          strokeStatus = `Your peak performance of ${peak} WA points is currently ${gapToCounty} pts below the County baseline of ${cTarget} pts. Core focus is on stroke mechanics and pacing strategy.`;
        }
...
export function calculateSquadHealth(input, attendance = [], sessions = [], results = [], period = 365, config = {}) {
  // Two call shapes are in use:
  //   STATS MODE  calculateSquadHealth(statsObj, squadObj)   ← every caller in the app
  //   RAW MODE    calculateSquadHealth(swimmersArray, attendance, sessions, results, period, config)
  //
  // In stats mode the squad lands in the `attendance` slot, so the weights were
  // read off an empty `config` and every squad was scored on the hardcoded
  // defaults — per-squad health weights set in Settings did nothing at all.
  let stats = input;
  ...
  if (Array.isArray(input)) {
    ...
    swimmers.forEach(s => {
      // `input` is an Array here, so the old `input.memberships` was always
      // undefined and reliability was computed with no session memberships.
      const swimmerMemberships = memberships.filter(m => m.swimmer_id === s.id);
      const rel = calculateReliability(s, attendance, sessions, results, period, (config.exemptions || []), swimmerMemberships);
      totalTraining += rel.percentage || 0;
      totalVolume += rel.volumePct || 0;
      totalMeets += rel.complianceRate || 0;
      ...
      if (sResults.length >= 2) {
        ...
        // A single null wa_pts made the whole squad's health score NaN.
        const first = sorted[0].wa_pts || 0;
        const last = sorted[sorted.length - 1].wa_pts || 0;
        totalVelocity += (last - first) / (sorted.length || 1);
      }
    });
  ...
  } else if (attendance && !Array.isArray(attendance) && typeof attendance === 'object') {
    // STATS MODE: the second argument is the squad, and the squad carries the
    // health weights.
    finalConfig = attendance;
  }
  ...
  // `|| 0` on avgVelocity: a NaN here propagates through Math.max and renders
  // the whole squad health score as NaN.
  const velocityScore = Math.min(100, Math.max(0, 50 + ((stats.avgVelocity || 0) * 2)));
...
/**
 * SINGLE canonical squad-stats computation shared by the squads registry
 * (pages/squads.js), the squad detail page (pages/squad/[id].js) and the
 * dashboard squad cards (pages/dashboard.js) so all three render IDENTICAL
 * numbers for the same squad + period.
 *
 * Swimmer set: active AND non-exempt (excludes is_active === false and is_exempt),
 * matching the detail page.
 *
 *  - training / volume : average of rel.percentage / rel.volumePct over that set
 *  - meets / compliance: % of swimmers meeting target, where
 *        target = squad.target_training_percent || 75 and a swimmer is "met" when
 *        rel.complianceRate >= 100 && (rel.percentage >= target || rel.volumePct >= target)
 *  - avgPts            : average peak wa_pts over the period
 *  - avgVelocity       : average per-swimmer velocity (recent-half vs prior-half)
 *  - health            : calculateSquadHealth(...).total with the squad config
 *
 * @returns {{ training:number, volume:number, meets:number, compliance:number,
 *             health:number, avgPts:number, avgVelocity:number, healthDetail:object }}
 */
export function computeSquadStats(squad, squadSwimmers, { attendance = [], sessions = [], results = [], memberships = [], exemptions = [], period = 365 } = {}) {
  const target = (squad && squad.target_training_percent) || 75;
  const now = new Date();
  ...
  // Canonical swimmer set: active AND non-exempt.
  const set = (squadSwimmers || []).filter(s => s.is_active !== false && !s.is_exempt);

  const per = set.map(s => {
    const swAtt = (attendance || []).filter(a => a.swimmer_id === s.id);
    const swRes = (results || []).filter(r => r.swimmer_id === s.id);
    const swMem = (memberships || []).filter(m => m.swimmer_id === s.id);
    // Inject the squad so calculateReliability reads the squad's targets
    // consistently, regardless of whether the caller hydrated swimmer.squads.
    const rel = calculateReliability({ ...s, squads: squad }, swAtt, sessions, swRes, period, exemptions, swMem);

    const inPeriod = swRes.filter(r => new Date(r.date) >= velPriorStart);
    const peakWA = inPeriod.length ? Math.max(...inPeriod.map(r => r.wa_pts || 0)) : 0;
    ...
    const velocity = Math.round(mean(recentPts) - mean(priorPts));
    const isMet = rel.complianceRate >= 100 && (rel.percentage >= target || rel.volumePct >= target);

    return { percentage: rel.percentage, volumePct: rel.volumePct, peakWA, velocity, isMet };
  });

  const n = per.length || 1;
  // `|| 0` throughout: percentage is null for swimmers with no swimmable weeks.
  const training = Math.round(per.reduce((a, b) => a + (b.percentage || 0), 0) / n);
  const volume = Math.round(per.reduce((a, b) => a + (b.volumePct || 0), 0) / n);
  const avgVelocity = Math.round(per.reduce((a, b) => a + (b.velocity || 0), 0) / n);
  const avgPts = Math.round(per.reduce((a, b) => a + (b.peakWA || 0), 0) / n);
  const compliance = per.length ? Math.round((per.filter(s => s.isMet).length / per.length) * 100) : 0;

  const healthDetail = calculateSquadHealth(
    { avgTraining: training, avgVolume: volume, avgVelocity, complianceRate: compliance },
    squad || {}
  );
