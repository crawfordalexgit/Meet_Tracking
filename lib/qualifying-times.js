...
import { calculateWAPoints } from './wa-points.js';
...
export function getKentBenchmark(age, gender, event, level = 'COUNTY') {
  ...
  const eventData = STANDARDS[l]?.[g]?.[event];
  if (eventData) {
    // Guarded: an event whose ages map lacks all four fallback keys left
    // ageData undefined and `ageData.autoSC` took down the predictor/report
    // render for that swimmer. Fall through to the averaged fallback instead.
    const ageData = eventData.ages?.[lookupAge] || eventData.ages?.[17] || eventData.ages?.[12] || eventData.ages?.[11];
    if (ageData) {
      const livePts = calculateWAPoints(ageData.autoSC, event, g, 'SCM');
      return livePts > 0 ? livePts : ageData.pts;
    }
  }
...
export function getCategoryBenchmark(age, gender, stroke, level = 'COUNTY') {
  ...
  // If a full event name is passed (contains a digit), normalize it to STANDARDS key format
  // e.g. "50 Freestyle" → "50 Free", "200 Individual Medley" → "200 IM"
  if (stroke && /\d/.test(stroke)) {
    const normalized = stroke
      .replace(/^Event\s+\d+\s*/i, '')
      .replace(/\b(boys?|girls?|mens?|womens?|open)\b\s*/gi, '')
      .replace(/\d{1,2}\s*[-–]\s*\d{1,3}\s*/g, '')
      ...
      .replace(/\s*(lc|sc)\b\s*/gi, ' ')
      .replace(/\s*meters?\s*/gi, ' ')
      .replace(/\d{1,2}\s+year\s*olds?\s*/gi, '')
      .replace(/\s+/g, ' ')
      .trim();
    if (standardSet[normalized]) {
      const d = standardSet[normalized].ages[lookupAge] || standardSet[normalized].ages[17] || standardSet[normalized].ages[12] || standardSet[normalized].ages[11];
      if (d) {
        const livePts = calculateWAPoints(d.autoSC, normalized, g, 'SCM');
        return livePts > 0 ? livePts : d.pts;
      }
    }
  }
  ...
  Object.entries(standardSet).forEach(([eventName, data]) => {
    if (eventName.toLowerCase().includes(keyword.toLowerCase())) {
      const ageData = data.ages[lookupAge] || data.ages[2] || data.ages[4] || data.ages[5] || data.ages[6] || data.ages[7] || data.ages[1] || data.ages[3];
      if (ageData) {
        const livePts = calculateWAPoints(ageData.autoSC, eventName, g, 'SCM');
        totalPts += livePts > 0 ? livePts : ageData.pts;
        count++;
      }
    }
  });
