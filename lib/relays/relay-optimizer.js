...
import { BASE_TIMES_2024 } from '../wa-points.js';
...
/**
 * Convert a 50m long-course time to a short-course estimate using the ratio of
 * World Aquatics base times (SCM/LCM) for that stroke and sex — a principled,
 * stroke-specific factor (SC is faster off the extra turn). Falls back to a flat
 * 0.98 if the base table lacks the event. Used only when a swimmer has no SCM
 * time for a leg; real SCM times always take priority.
 */
function lcToScEstimate(lcTime, leg, sx) {
  const ev = LEG_EVENT[leg];
  const scm = BASE_TIMES_2024?.SCM?.[sx]?.[ev];
  const lcm = BASE_TIMES_2024?.LCM?.[sx]?.[ev];
  const factor = scm && lcm ? scm / lcm : 0.98;
  return lcTime * factor;
}
 ...
 * @param {Object} pbsBySwimmer  { [swimmerId]: [{ event, course, time_seconds }] } — 'S' and 'L'
 * @param {number} ageYear  meet age year (2026)
 * @param {Object} [opts]
 * @param {boolean} [opts.lcFallback=true]  when no SCM 50 exists, use a converted LC time
 * @returns {Array} [{ id, name, age, sex, times:{stroke:sec}, converted:{stroke:bool} }]
 */
export function buildSwimmerPool(swimmers, pbsBySwimmer, ageYear, { lcFallback = true } = {}) {
  return (swimmers || []).map((s) => {
    const pbs = pbsBySwimmer[s.id] || [];
    const sx = sex(s.gender);
    const times = {};
    const converted = {};
    for (const leg of STROKES) {
      const wantNorm = NORM_LEG_EVENT[leg];
      let bestS = null, bestL = null;
      for (const pb of pbs) {
        if (normalizeEvent(pb.event || '') !== wantNorm) continue;
        const t = Number(pb.time_seconds);
        if (!t || t <= 0) continue;
        if (pb.course === 'L') { if (bestL === null || t < bestL) bestL = t; }
        else { if (bestS === null || t < bestS) bestS = t; } // treat anything non-'L' as short course
      }
      if (bestS !== null) { times[leg] = bestS; converted[leg] = false; }
      else if (lcFallback && bestL !== null) { times[leg] = lcToScEstimate(bestL, leg, sx); converted[leg] = true; }
      else { times[leg] = null; converted[leg] = false; }
    }
    return {
      id: s.id,
