...
/**
 * Applies a Pubertal Smoothing Factor to WA points based on Vorontsov's LTAD model.
 * Girls 10-13 are biologically ~2 years ahead of boys, inflating their WA points.
 */
export function getNormalizedWA(points, age, gender, isEnabled = false) {
  if (!isEnabled || !points || !age || !gender) return points;
  const numPoints = parseFloat(points);
  if (isNaN(numPoints)) return points;

  // Apply +/- 10% smoothing during the critical PHV window
  if (gender === 'F' && age >= 10 && age <= 13) return Math.round(numPoints * 0.90);
  if (gender === 'M' && age >= 12 && age <= 14) return Math.round(numPoints * 1.10);
  return numPoints;
}
