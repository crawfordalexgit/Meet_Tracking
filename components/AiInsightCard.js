    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (legacyData && legacyData.length > 0) {
          ...
          // Auto-generate if not run before
          if (!hasAutoGenerated.current[type]) {
            hasAutoGenerated.current[type] = true;
      ...
      <div className="glass-card animate-fade-in no-print" style={{ padding: '1.5rem 2rem' }}>
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col gap-2" style={{ flex: '1 1 20%', minWidth: '160px', width: '100%' }}>
            {isTraining ? (
              <button className="btn-premium-action w-full" onClick={() => generateInsight('training')} style={{ padding: '10px 16px', fontSize: '0.8rem' }}>
                <span>✨</span> Run Workload Analysis
              </button>
            ) : isPathway ? (
              <button className="btn-premium-action w-full" onClick={() => generateInsight('pathway')} style={{ padding: '10px 16px', fontSize: '0.8rem' }}>
                <span>✨</span> Run Pathway Analysis
              </button>
            ) : (
              <>
                <button className="btn-premium-action w-full" onClick={() => generateInsight('general')} style={{ padding: '10px 16px', fontSize: '0.8rem' }}>
                  <span>✨</span> Run Technical Analysis
                </button>
                <button 
                  className="btn-premium-intel w-full" 
                ...
                >
                  <span>🔥</span> Run Burnout Check
                </button>
                <button className="btn-premium-intel w-full" onClick={generateParentInsight} style={{ borderColor: 'var(--accent-amber)', color: 'var(--accent-amber)', background: 'rgba(245,158,11,0.1)', padding: '10px 16px', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>👪</span> Run Parent Audit
                </button>
              </>
            )}
          </div>
