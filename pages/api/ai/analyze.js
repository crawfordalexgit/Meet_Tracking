...
import { analyzeSwimmer, analyzeSquad } from '../../../lib/ai';

export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const { swimmerId, squadId, type = 'general', squadName, stats, strokeData, trend, swimmers } = req.body;
  ...
  try {
    ...
    // 3. Call Gemini
    const analysis = await analyzeSwimmer(dna, type, feedback);
